...
class EquiposController extends AbstractController
{
    ...
    public function new(Request $request, EquiposRepository $equiposRepository): Response
    {
        $equipo = new Equipos();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $equiposRepository->save($equipo, true);
    ...
    public function edit(Request $request, Equipos $equipo, EquiposRepository $equiposRepository): Response
    {
        $form = $this->createForm(EquiposType::class, $equipo);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, Equipos $equipo, EquiposRepository $equiposRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$equipo->getId(), $request->request->get('_token'))) {
            $equiposRepository->remove($equipo, true);
        }
